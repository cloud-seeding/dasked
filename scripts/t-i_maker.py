...
def process_month(group, year, month, variables, root, fldr):
    """
    ...
    os.makedirs(f"./{fldr}", exist_ok=True)

    output_file = f"./{fldr}/backdated_{year}{str(month).zfill(2)}.csv"
...
def main(i):
    ...
    args_list = [(group, year, month, variables, root, fldr)
